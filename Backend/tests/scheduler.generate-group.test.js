...
describe("SchedulerEngine.genererGroupe", () => {
  ...
  test("rejette un identifiant de groupe invalide", async () => {
    await expect(SchedulerEngine.genererGroupe({ idGroupe: "abc" })).rejects.toMatchObject({
  ...
  test("annule la transaction quand aucune session active n'est disponible", async () => {
    queryMock.mockImplementationOnce(async (sql) => {
      const normalizedSql = String(sql).replace(/\s+/g, " ").trim();
    ...
    await expect(SchedulerEngine.genererGroupe({ idGroupe: 12 })).rejects.toMatchObject({
      message: "Aucune session active. Activez une session dans Pilotage sessions.",
      statusCode: 422,
    });
  ...
  test("annule la transaction quand le groupe cible est introuvable", async () => {
  ...
  test("retourne un diagnostic non planifie quand meme avec la tentative assouplie", async () => {
    queryMock.mockImplementation(async (sql) => {
      const normalizedSql = String(sql).replace(/\s+/g, " ").trim();
      ...
      if (normalizedSql.includes("FROM absences_professeurs")) {
        return [[{ id_professeur: 10, date_debut: "2026-01-15", date_fin: "2026-01-16", type: "Maladie" }]];
      }

      if (normalizedSql.includes("FROM salles_indisponibles")) {
        return [[{ id_salle: 5, date_debut: "2026-01-15", date_fin: "2026-01-16", raison: "Maintenance" }]];
      }
    ...
    const optimisationSpy = jest
      ...
      .mockImplementation(({ affectationsReprises }) => ({
        placementsOptimises: [],
        scoringBefore: {
          modes: {
            equilibre: { scoreGlobal: 10 },
          },
        },
        scoringAfter: {
          modes: {
    ...
    try {
      ...
      expect(rapport.nb_cours_planifies).toBe(0);
      expect(rapport.nb_cours_non_planifies).toBe(1);
      expect(rapport.non_planifies[0]).toMatchObject({
        id_cours: 401,
        groupe: "GPI-E4-2",
        seance: 1,
      });
      expect(rapport.details.nb_reprises_rattachees).toBe(1);
      expect(rapport.details.optimisation_locale.scoreAvantOptimisationLocale).toBe(10);
